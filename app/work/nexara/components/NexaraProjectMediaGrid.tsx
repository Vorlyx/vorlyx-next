...
interface NexaraProjectMediaGridProps {
  gridItems: GridItem[];
}
...
// Responsive image sizes for Next.js optimization
const IMAGE_SIZES =
  "(max-width: 767px) 100vw, (max-width: 1023px) 50vw, (max-width: 1366px) 50vw, 960px";
...
export default function NexaraProjectMediaGrid({
  gridItems,
}: NexaraProjectMediaGridProps) {
  ...
  const gridLayout: LayoutRow[] = useMemo(
    () => [
      ...
      { items: [sortedItems[3], sortedItems[4]], cols: 2, widths: [3, 4] },
      { items: [sortedItems[5], sortedItems[6]], cols: 2, widths: [4, 3] },
      { items: [sortedItems[7]], cols: 1 },
      { items: [sortedItems[8], sortedItems[9]], cols: 2, widths: [4, 3] },
      { items: [sortedItems[10]], cols: 1 },
    ],
    [sortedItems]
  );
